...
| 11  | Модерация | Управление каталогом | Потребитель-поставщик (Customer-Supplier) | OHS (API) + ACL | REST API: POST /api/v1/books/{id}/moderate → { status: "blocked", reason } |
| 12  | Модерация | Персонализация | Сервис с открытым протоколом (OHS) | Публикация событий | Событие: BookModerated { bookId, status } |